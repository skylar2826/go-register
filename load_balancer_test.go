...
import (
	...
	grpc2 "micro/grpc"
	"micro/loadBalancer/round_robin"
	"micro/proto/gen"
	"micro/register"
	"micro/register/etcd"
	"sync"
...
func TestRoundRobinServer(t *testing.T) {
	...
	wg.Add(3)
	go func() {
		err = s.Start("127.0.0.1:8080", &ServerConfig{Weight: 10})
		wg.Done()
		if err != nil {
			t.Fatal(err)
			return
	...
	go func() {
		err = s.Start("127.0.0.1:8081", &ServerConfig{Weight: 12})
		wg.Done()
		if err != nil {
			t.Fatal(err)
			return
	...
	go func() {
		err = s.Start("127.0.0.1:8082", &ServerConfig{Weight: 13})
		wg.Done()
		if err != nil {
			t.Fatal(err)
			return
...
func TestRoundRobinClient(t *testing.T) {
	...
	c := NewClient(WithInSecure(), WithClientRegister(r), WithBalancer("TEST_DEMO_WEIGHT_ROUND_ROBIN"))
